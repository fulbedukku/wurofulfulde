...
export const SongCard = ({ song }) => {
  ...
  return (
    <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 m-2">
      <Link to={`/song/${encodedTitle}`}>
        <img className="rounded-t-lg" src={`${import.meta.env.BASE_URL}`+ song.image} alt={song.title} />
      </Link>
      <div className="p-5">
        <Link to={`/song/${encodedTitle}`}>
          <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{song.title}</h5>
        </Link>
        <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">{song.description}</p>
        <Link
          to={`/song/${encodedTitle}`}
